...
class Game 
    ...
    def initialize(n_players)
        ...
        for i in 0..n_players-1
            @players[i] = Player.new(i,0.1,0.1)
            @players[i].set_pos(Dice.random_pos(@@FILAS_MAX),Dice.random_pos(@@COLUMNAS_MAX))
        end
        ...
        @monsters = Array.new(@@NUM_MONSTERS){Monster.new("Monstruo",Dice.random_intelligence,Dice.random_strength)}
    ...
    def get_game_state
        @game_state = Game_state.new(@labyrinth,@players,@monsters,@current_player,@labyrinth.have_a_winner,@log)
    end
    ...
    def configure_labyrinth()
        @monsters.each do |monster|
            @labyrinth.add_monster(Dice.random_pos(@labyrinth.n_rows),Dice.random_pos(@labyrinth.n_cols),monster)
        end
        for i in 0..1
            @labyrinth.add_block(Orientation::HORIZONTAL,Dice.random_pos(@@FILAS_MAX),Dice.random_pos(@@COLUMNAS_MAX),3)
        end
        for i in 0..1
            @labyrinth.add_block(Orientation::VERTICAL,Dice.random_pos(@@FILAS_MAX),Dice.random_pos(@@COLUMNAS_MAX),3)
        end
    end
    ...
    def log_rounds(rounds, max)
        @log << "Se han producido " << rounds.to_s << "/" << max.to_s << "rondas de combate.\n"
    end
end